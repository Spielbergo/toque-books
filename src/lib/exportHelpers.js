...
export function exportJSON(state, userProfile, fyKey) {
  ...
  download(`northbooks-backup-${fyKey}.json`, JSON.stringify(payload, null, 2), 'application/json');
}
...
<!-- CRA T2 Corporate Income Tax Return Summary — Generated by NorthBooks -->
...
export function exportT1XML(userProfile, fyKey, companyName) {
  ...
  const xml = `<?xml version="1.0" encoding="UTF-8"?>
<!-- CRA T1 Personal Income Tax Return Summary — Generated by NorthBooks -->
...
<TaxReturn software="NorthBooks" year="${year}">
  <Taxpayer>
    <SIN></SIN>
...
export function exportSage50CSV(state, fyKey) {
  ...
  for (const exp of expenses) {
    rows.push(csvRow(['Purchase Journal', exp.date ?? '', exp.id?.slice(0, 8) ?? '', 'NorthBooks', exp.description ?? exp.category ?? '', (exp.amount ?? 0).toFixed(2), '', '5100', exp.category ?? 'General Expense']));
  }

  for (const inv of invoices.filter(i => i.status === 'paid' || i.status === 'sent')) {
    rows.push(csvRow(['Sales Journal', inv.issueDate ?? '', inv.invoiceNumber ?? '', 'NorthBooks', `Invoice ${inv.invoiceNumber ?? ''} — ${inv.client?.name ?? ''}`, '', (inv.subtotal ?? 0).toFixed(2), '4100', 'Revenue']));
    if ((inv.hstAmount ?? 0) > 0) {
      rows.push(csvRow(['Sales Journal', inv.issueDate ?? '', inv.invoiceNumber ?? '', 'NorthBooks', 'HST/GST Collected', '', (inv.hstAmount ?? 0).toFixed(2), '2200', 'HST/GST Payable']));
    }
  }
...
export function exportFutureTaxT1Info(userProfile, fyKey) {
  ...
  const lines = [
    `NorthBooks — FutureTax T1 Personal Income Summary`,
    `Tax Year: ${year}`,
